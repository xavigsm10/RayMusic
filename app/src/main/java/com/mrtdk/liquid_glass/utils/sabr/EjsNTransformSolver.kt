package com.mrtdk.liquid_glass.utils.sabr
...
import com.mrtdk.liquid_glass.utils.cipher.PlayerJsFetcher
...
object EjsNTransformSolver {
    private const val TAG = "EjsNSolver"
    ...
    suspend fun transformNParamInUrl(url: String): String {
        ...
        if (nMatch == null) {
            Log.d(TAG, "No 'n' parameter in stream URL")
            return url
        }
        val nValue = Uri.decode(nMatch.groupValues[1])
        Log.d(TAG, "stream n-param: $nValue")
        ...
        return withContext(NonCancellable) {
            ...
            try {
                ...
                Log.d(TAG, "stream n-param transformed: $nValue -> $transformed")
                url.replaceFirst(
                    Regex("([?&])n=[^&]+"),
                    "$1n=${Uri.encode(transformed)}"
                )
            } catch (e: Exception) {
                Log.e(TAG, "stream n-transform failed: ${e.message}")
                url
            }
        }
    }
    ...
    private suspend fun getOrCreateSolver(): SolverWebView? {
        ...
        return withContext(NonCancellable) {
            ...
            try {
                ...
                Log.e(TAG, "Failed to create EJS solver: ${e.message}")
                null
            }
        }
    }
