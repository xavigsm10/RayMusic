...
object PlayerJsFetcher {
    private const val TAG = "CipherFetcher"
    ...
    suspend fun getPlayerJs(forceRefresh: Boolean = false): Pair<String, String>? = withContext(Dispatchers.IO) {
        try {
            ...
            if (!forceRefresh) {
                ...
                if (cached != null) {
                    Log.d(TAG, "Using cached player JS (hash=${cached.second})")
            ...
            Log.d(TAG, "Downloaded player JS: ${playerJs.length} chars")
            ...
            Log.e(TAG, "getPlayerJs exception: ${e.message}")
            null
        }
    }
    ...
    fun invalidateCache() {
        try {
            ...
            Log.e(TAG, "Failed to invalidate cache: ${e.message}")
        }
    }
    ...
    private fun readFromCache(): Pair<String, String>? {
        try {
            ...
            if (System.currentTimeMillis() - timestamp > CACHE_TTL_MS) {
                Log.d(TAG, "Cache expired (hash=$hash)")
                return null
            }
    ...
    private fun writeToCache(hash: String, playerJs: String) {
        try {
            ...
            Log.e(TAG, "Error writing cache: ${e.message}")
        }
    }
