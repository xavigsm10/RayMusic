...
object FunctionNameExtractor {
    private const val TAG = "CipherFnExtract"
    ...
    fun extractSigFunctionInfo(playerJs: String): SigFunctionInfo? {
        for ((index, pattern) in SIG_FUNCTION_PATTERNS.withIndex()) {
            ...
            if (match != null) {
                ...
                Log.d(TAG, "Sig function found with pattern $index: $name (constantArg=$constArg)")
            ...
            if (match != null) {
                when (index) {
                    0 -> {
                        ...
                        Log.d(TAG, "N-function found with pattern $index: $name (arrayIndex=$arrayIdx)")
                    ...
                    1 -> {
                        ...
                        val name = match.groupValues[1]
                        Log.d(TAG, "N-function found with pattern $index: $name")
