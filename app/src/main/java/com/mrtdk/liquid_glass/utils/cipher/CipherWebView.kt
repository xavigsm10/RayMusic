...
class CipherWebView private constructor(
        ...
        webView.webChromeClient = object : WebChromeClient() {
            override fun onConsoleMessage(m: ConsoleMessage): Boolean {
                if (m.message().contains("Uncaught") && !m.message().contains("is not defined")) {
                    Log.e(TAG, "WebView JS error: ${m.message()} at ${m.sourceId()}:${m.lineNumber()}")
                }
    ...
    private fun loadPlayerJsFromFile() {
        ...
        Log.d(TAG, "Loading player JS from file (${playerJs.length} chars), exporting sig=$sigFuncName, nFunc=$nFuncName[$nArrayIdx]")
...
<script src="player.js"
    ...
    fun onNDiscoveryDone(funcName: String, info: String) {
        if (funcName.isNotEmpty()) {
            Log.d(TAG, "N-function DISCOVERED: $funcName ($info)")
            discoveredNFuncName = funcName
            nFunctionAvailable = true
        } else {
            Log.e(TAG, "N-function NOT found ($info)")
    ...
    fun onPlayerJsLoaded() {
        Log.d(TAG, "Player JS loaded, n-func=${discoveredNFuncName ?: "none"}")
        initContinuation.resume(this)
    }

    @JavascriptInterface
    fun onPlayerJsError(error: String) {
        Log.e(TAG, "Player JS load error: $error")
    ...
    fun onSigResult(result: String) {
        Log.d(TAG, "Signature deobfuscated: ${result.take(30)}...")
    ...
    fun onSigError(error: String) {
        Log.e(TAG, "Signature deobfuscation error: $error")
