 ...
 * onDragStopped reports whether the drag was cancelled, kotlin.time.Clock replaced by
 * SystemClock (Kotlin 2.1 compatible), moveToValue added.
 */
...
internal class DampedDragAnimation(
    ...
    val onDragStopped: DampedDragAnimation.(cancelled: Boolean) -> Unit,
    ...
    /** Like [updateValue] but without feeding the velocity, so a jump does not stretch the glass. */
    fun moveToValue(value: Float) {
        val targetValue = value.coerceIn(valueRange)
        animationScope.launch { valueAnimation.animateTo(targetValue, valueAnimationSpec) }
    }
