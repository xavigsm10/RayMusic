...
function initPlayerEvents() {
  ...
  if (btnLyrics) {
    btnLyrics.addEventListener('click', (e) => {
      e.preventDefault();
      toggleFullscreenPlayer('lyrics');
    });
  }

  if (songArtworkMini) {
    songArtworkMini.style.cursor = 'pointer';
    songArtworkMini.addEventListener('click', () => {
      toggleFullscreenPlayer('centered');
    });
  }
}

// Fullscreen Immersive Player Overlay (Matching Image 2 with Frutiger Aero dynamic fluid colors)
let activeFullscreenOverlay = null;

function toggleFullscreenPlayer(mode = 'centered') {
  if (activeFullscreenOverlay) {
    closeFullscreenPlayerOverlay();
    return;
  }

  const track = (currentQueue && activeIndex >= 0 && activeIndex < currentQueue.length)
    ? currentQueue[activeIndex]
    : { title: "I'll Be There", artist: "Jackson 5", album: "Michael: Songs From The Motion Picture", artwork: "https://images.unsplash.com/photo-1514525253161-7a46d19cd819?w=800" };

  const artworkUrl = upgradeThumbQuality(track.artwork || songArtworkMini?.src || "");

  const overlay = document.createElement('div');
  overlay.className = "fullscreen-player-overlay";
  ...
  overlay.style.background = "#0e0204";

  // Dynamic Frutiger Aero Moving Fluid Background
  const aeroBg = document.createElement('div');
  aeroBg.className = "aero-fluid-backdrop";
  aeroBg.style.background = `radial-gradient(circle at 20% 30%, #e62e4d 0%, transparent 60%), radial-gradient(circle at 80% 70%, #991b32 0%, transparent 65%), radial-gradient(circle at 50% 50%, #4a0d18 0%, #120406 100%)`;

  extractDominantColor(artworkUrl, (colorHex) => {
    aeroBg.style.background = `radial-gradient(circle at 25% 25%, ${colorHex} 0%, transparent 55%), radial-gradient(circle at 75% 75%, #ff2d55 0%, transparent 60%), radial-gradient(circle at 50% 50%, #200509 0%, #0d0203 100%)`;
  });

  overlay.appendChild(aeroBg);

  // Top Left X Close Button (Matching Image 2)
  const closeBtn = document.createElement('button');
  closeBtn.id = "fullscreen-close-btn";
  closeBtn.title = "Cerrar reproductor";
  ...
  // Center Content Area: Artwork + Controls Block (Matching Image 2)
  const centerBox = document.createElement('div');
  centerBox.style.position = "relative";
  centerBox.style.zIndex = "2";
  centerBox.style.display = "flex";
  centerBox.style.flexDirection = "column";
  centerBox.style.alignItems = "center";
  centerBox.style.width = "480px";

  const artworkImg = document.createElement('img');
  artworkImg.src = artworkUrl;
  artworkImg.style.width = "480px";
  artworkImg.style.height = "480px";
  artworkImg.style.borderRadius = "20px";
  artworkImg.style.objectFit = "cover";
  artworkImg.style.boxShadow = "0 24px 60px rgba(0,0,0,0.65)";
  artworkImg.style.marginBottom = "24px";

  // Controls & Metadata Container below artwork matching Image 2
  const metaBox = document.createElement('div');
  metaBox.style.width = "100%";
  metaBox.style.display = "flex";
  metaBox.style.flexDirection = "column";
  metaBox.style.gap = "14px";
  metaBox.style.color = "white";

  metaBox.innerHTML = `
    <!-- Row 1: Track Info + Star & More -->
    <div style="display: flex; align-items: center; justify-content: space-between; width: 100%;">
      <div style="display: flex; flex-direction: column; overflow: hidden; white-space: nowrap; max-width: 380px;">
        <span style="font-size: 17px; font-weight: 800; color: white; text-overflow: ellipsis; overflow: hidden;" id="fs-track-title">${escapeHtmlAttr(track.title)}</span>
        <span style="font-size: 12.5px; font-weight: 600; color: rgba(255,255,255,0.7); text-overflow: ellipsis; overflow: hidden; margin-top: 2px;" id="fs-track-sub">${escapeHtmlAttr(track.artist)}${track.album ? ' — ' + escapeHtmlAttr(track.album) : ''}</span>
      </div>
      <div style="display: flex; gap: 10px; align-items: center;">
        <button style="background: rgba(255,255,255,0.12); border: none; width: 32px; height: 32px; border-radius: 50%; color: white; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 14px;" title="Favoritos">☆</button>
        <button style="background: rgba(255,255,255,0.12); border: none; width: 32px; height: 32px; border-radius: 50%; color: white; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 14px;" title="Más opciones">•••</button>
      </div>
    </div>

    <!-- Row 2: Progress Line & Timers -->
    <div style="display: flex; flex-direction: column; gap: 6px; width: 100%;">
      <div style="width: 100%; height: 4px; background: rgba(255,255,255,0.25); border-radius: 2px; position: relative; cursor: pointer;" id="fs-timeline-bar">
        <div style="height: 100%; width: 30%; background: #ffffff; border-radius: 2px;" id="fs-timeline-progress"></div>
        <div style="width: 10px; height: 10px; border-radius: 50%; background: #ffffff; position: absolute; top: -3px; left: 30%; transform: translateX(-50%); shadow: 0 2px 6px rgba(0,0,0,0.5);" id="fs-timeline-handle"></div>
      </div>
      <div style="display: flex; justify-content: space-between; font-size: 11px; font-weight: 600; color: rgba(255,255,255,0.6);">
        <span id="fs-time-elapsed">${timeElapsedLabel?.textContent || '0:20'}</span>
        <span id="fs-time-remaining">${timeRemainingLabel?.textContent || '-1:10'}</span>
      </div>
    </div>

    <!-- Row 3: Centered Playback Controls -->
    <div style="display: flex; justify-content: center; align-items: center; gap: 24px; width: 100%; margin-top: 2px;">
      <button style="background: none; border: none; color: rgba(255,255,255,0.7); cursor: pointer;" id="fs-ctrl-shuffle" title="Aleatorio"><svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M10.59 9.17L5.41 4 4 5.41l5.17 5.17 1.42-1.41zM14.5 4l2.04 2.04L4 18.59 5.41 20 17.96 7.46 20 9.5V4h-5.5zm.33 9.41l-1.41 1.41 3.13 3.13L14.5 20H20v-5.5l-2.04 2.04-3.13-3.13z"/></svg></button>
      <button style="background: none; border: none; color: white; cursor: pointer;" id="fs-ctrl-prev" title="Anterior"><svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M6 6h2v12H6zm3.5 6l8.5 6V6z"/></svg></button>
      <button style="width: 40px; height: 40px; border-radius: 50%; background: white; color: black; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center;" id="fs-ctrl-play" title="Reproducir / Pausa">
        <svg viewBox="0 0 24 24" width="22" height="22" fill="currentColor"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/></svg>
      </button>
      <button style="background: none; border: none; color: white; cursor: pointer;" id="fs-ctrl-next" title="Siguiente"><svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M6 18l8.5-6L6 6v12zM16 6v12h2V6z"/></svg></button>
      <button style="background: none; border: none; color: rgba(255,255,255,0.7); cursor: pointer;" id="fs-ctrl-repeat" title="Repetir"><svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M7 7h10v3l4-4-4-4v3H5v6h2V7zm10 10H7v-3l-4 4 4 4v-3h12v-6h-2v4z"/></svg></button>
    </div>

    <!-- Row 4: Volume Bar -->
    <div style="display: flex; align-items: center; gap: 12px; width: 100%; margin-top: 6px;">
      <svg viewBox="0 0 24 24" width="16" height="16" fill="rgba(255,255,255,0.7)"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02z"/></svg>
      <div style="flex: 1; height: 3px; background: rgba(255,255,255,0.25); border-radius: 2px; position: relative; cursor: pointer;">
        <div style="height: 100%; width: 70%; background: #ffffff; border-radius: 2px;"></div>
        <div style="width: 10px; height: 10px; border-radius: 50%; background: #ffffff; position: absolute; top: -3.5px; left: 70%; transform: translateX(-50%);"></div>
      </div>
    </div>
  `;

  centerBox.appendChild(artworkImg);
  centerBox.appendChild(metaBox);
  ...
  closeBtn.onclick = closeFullscreenPlayerOverlay;
  metaBox.querySelector('#fs-ctrl-play').onclick = () => {
    if (playBtn) playBtn.click();
  };
}

function closeFullscreenPlayerOverlay() {
  if (activeFullscreenOverlay) {
    activeFullscreenOverlay.remove();
    activeFullscreenOverlay = null;
  }
}

// Function to render queue list with full-width large artwork in right sidebar (Matching Image 3 specs)
function renderQueue() {
  const container = document.getElementById('queue-list');
  const countLabel = document.getElementById('queue-item-count');
  if (!container) return;

  container.innerHTML = '';
  if (countLabel) countLabel.textContent = `${currentQueue.length} items`;

  if (currentQueue.length === 0) {
    container.innerHTML = `<div style="padding: 20px; text-align: center; color: rgba(255,255,255,0.5); font-size: 13px;">Sin canciones en la cola</div>`;
    return;
  }

  // Section 1: Top Currently Playing Hero Block (No capsule wrapper, full width large image, title & artist below, no "Song •")
  if (activeIndex >= 0 && activeIndex < currentQueue.length) {
    const np = currentQueue[activeIndex];
    const npBlock = document.createElement('div');
    npBlock.style.display = "flex";
    npBlock.style.flexDirection = "column";
    npBlock.style.width = "100%";
    npBlock.style.marginBottom = "24px";
    npBlock.style.cursor = "pointer";

    npBlock.innerHTML = `
      <img src="${upgradeThumbQuality(np.artwork)}" style="width: 100%; aspect-ratio: 1/1; border-radius: 18px; object-fit: cover; box-shadow: 0 14px 35px rgba(0,0,0,0.55); transition: transform 0.2s ease;">
      <span style="font-size: 17px; font-weight: 900; color: #ffffff; margin-top: 12px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; letter-spacing: -0.01em;">${escapeHtmlAttr(np.title)}</span>
      <span style="font-size: 13px; font-weight: 600; color: rgba(255,255,255,0.7); margin-top: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${escapeHtmlAttr(np.artist)}</span>
    `;

    npBlock.onclick = () => toggleFullscreenPlayer('centered');
    container.appendChild(npBlock);
  }

  // Section 2: AutoPlay / Playing Next
  const header = document.createElement('div');
  header.style.fontSize = "13.5px";
  header.style.fontWeight = "800";
  header.style.color = "white";
  header.style.marginBottom = "12px";
  header.style.paddingLeft = "2px";
  header.innerHTML = `∞ AutoPlay <span style="font-size: 11.5px; font-weight: 500; color: rgba(255,255,255,0.6); display: block; margin-top: 2px;">Similar music will continue playing.</span>`;
  ...
  currentQueue.forEach((track, idx) => {
    ...
    row.innerHTML = `
      <img src="${upgradeThumbQuality(track.artwork)}" style="width: 44px; height: 44px; border-radius: 10px; object-fit: cover; flex-shrink: 0;">
      <div style="display: flex; flex-direction: column; overflow: hidden; white-space: nowrap; flex-grow: 1;">
        <span style="font-size: 13.5px; font-weight: 700; color: white; text-overflow: ellipsis; overflow: hidden;">${escapeHtmlAttr(track.title)}</span>
...
function cleanupArtistRightPanelBackground() {
  const rightPanel = document.querySelector('.right-panel');
  if (rightPanel) {
    const bgContainer = rightPanel.querySelector('#right-panel-bg-container');
    if (bgContainer) bgContainer.remove();
    const blurOverlay = rightPanel.querySelector('#right-panel-blur-overlay');
...
function setHeaderVisible(visible) {
  ...
  if (visible) {
    cleanupArtistRightPanelBackground();
  }
}
...
function renderPlaylistView(title, tracks, pageType = "Álbum", creator = "", artworkUrl = "") {
  ...
  const finalArtwork = artworkUrl || tracks[0]?.artwork || "https://images.unsplash.com/photo-1514525253161-7a46d19cd819?w=600";
  ...
  containerWrapper.style.padding = "28px 36px 48px 36px";
  containerWrapper.style.boxSizing = "border-box";
  containerWrapper.style.transition = "background 0.6s ease";
  containerWrapper.style.background = "linear-gradient(180deg, #5c3c1e 0%, #2a1b10 360px, #0e0906 100%)";

  extractDominantColor(finalArtwork, (colorHex) => {
    containerWrapper.style.background = `linear-gradient(180deg, ${colorHex} 0%, rgba(20,14,10,0.96) 380px, #0a0a0c 100%)`;
  });

  // Hero Section matching Image 4 attached by user
  const heroHeader = document.createElement('div');
  heroHeader.className = "album-header-hero";
  ...
  heroHeader.style.padding = "10px 0";

  heroHeader.innerHTML = `
    <img src="${finalArtwork}" style="width: 260px; height: 260px; border-radius: 20px; object-fit: cover; box-shadow: 0 16px 40px rgba(0,0,0,0.6); flex-shrink: 0; border: 1px solid rgba(255,255,255,0.12);">
    
    <div style="display: flex; flex-direction: column; justify-content: center; gap: 14px; flex: 1;">
      <h1 style="font-size: 34px; font-weight: 900; color: white; margin: 0; line-height: 1.15; letter-spacing: -0.5px;">${title}</h1>
      
      <!-- Artist Link Badge matching Image 4 -->
      <div style="display: flex; align-items: center; gap: 8px;">
        <div style="width: 22px; height: 22px; border-radius: 50%; background: #fa243c; display: flex; align-items: center; justify-content: center; color: white;">
        ...
        <span style="font-size: 15px; font-weight: 800; color: white; cursor: pointer;" onclick="event.stopPropagation(); loadArtistPage('${artistId}', '${artistName}')">${artistName}</span>
      </div>

      <!-- Sub-Pills Row: ÁLBUM · 2026 · X TEMAS -->
      <div style="display: flex; align-items: center; gap: 10px; margin-top: 2px;">
        <span style="background: rgba(255,255,255,0.18); backdrop-filter: blur(10px); padding: 4px 14px; border-radius: 14px; font-size: 11px; font-weight: 900; color: white; text-transform: uppercase;">ÁLBUM</span>
        <span style="background: rgba(255,255,255,0.18); backdrop-filter: blur(10px); padding: 4px 14px; border-radius: 14px; font-size: 11px; font-weight: 900; color: white;">2026</span>
        <span style="background: rgba(255,255,255,0.18); backdrop-filter: blur(10px); padding: 4px 14px; border-radius: 14px; font-size: 11px; font-weight: 900; color: white;">${tracks.length} TEMAS</span>
      </div>

      <!-- Action Buttons Row matching Image 4 -->
      <div style="display: flex; align-items: center; gap: 14px; margin-top: 8px;">
        <button id="btn-play-all" style="background: #ffffff; color: #000000; border: none; padding: 10px 28px; border-radius: 24px; font-size: 15px; font-weight: 900; cursor: pointer; display: flex; align-items: center; gap: 8px; box-shadow: 0 6px 20px rgba(0,0,0,0.35);">
          <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>
          <span>Reproducir</span>
        </button>

        <button id="btn-shuffle-all" style="background: rgba(255,255,255,0.16); border: 1px solid rgba(255,255,255,0.18); color: white; padding: 10px 22px; border-radius: 24px; font-size: 14px; font-weight: 800; cursor: pointer; display: flex; align-items: center; gap: 8px;">
        ...
        <button id="btn-header-add-lib" style="background: rgba(255,255,255,0.16); border: 1px solid rgba(255,255,255,0.18); color: white; padding: 10px 22px; border-radius: 24px; font-size: 14px; font-weight: 800; cursor: pointer; display: flex; align-items: center; gap: 6px;">
          <span>+ Agregar</span>
        </button>

        <button id="btn-header-more-options" style="background: rgba(255,255,255,0.16); border: 1px solid rgba(255,255,255,0.18); color: white; width: 42px; height: 42px; border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 18px; font-weight: 900;" title="Más opciones">
          •••
        </button>
      </div>
    </div>
...
function extractBottomStripColor(imageUrl, callback) {
  ...
  img.onload = () => {
    try {
      ...
      if (count > 0) {
        r = Math.min(30, Math.floor(r / count));
        g = Math.min(30, Math.floor(g / count));
        b = Math.min(32, Math.floor(b / count));
        callback(`rgb(${r}, ${g}, ${b})`, `rgba(${r}, ${g}, ${b}, 0.5)`);
      } else {
        callback('rgb(18, 19, 24)', 'rgba(18, 19, 24, 0.5)');
      }
    } catch(e) {
      callback('rgb(18, 19, 24)', 'rgba(18, 19, 24, 0.5)');
    }
  };
  img.onerror = () => callback('rgb(18, 19, 24)', 'rgba(18, 19, 24, 0.5)');
}
...
async function loadArtistPage(artistId, artistName, shouldPushHistory = true) {
  ...
  try {
    ...
    // Create wrapper element for artist page extending full width from x=0 under floating glass sidebar
    const artistWrapper = document.createElement('div');
    artistWrapper.id = "artist-page-wrapper";
    artistWrapper.style.position = "absolute";
    artistWrapper.style.top = "0";
    artistWrapper.style.left = "0";
    artistWrapper.style.width = "100%";
    artistWrapper.style.height = "100%";
    artistWrapper.style.overflowY = "auto";
    artistWrapper.style.overflowX = "hidden";
    artistWrapper.style.zIndex = "5";
    artistWrapper.style.padding = "0 0 90px 0";
    ...
    // Synchronize right panel background scroll position with artist page scroll
    artistWrapper.addEventListener('scroll', () => {
      const bgContainer = document.getElementById('right-panel-bg-container');
      if (bgContainer) {
        bgContainer.style.transform = `translateY(-${artistWrapper.scrollTop}px)`;
      }
    });
    ...
    heroSection.style.borderRadius = "0";
    ...
    heroSection.style.padding = "74px 32px 10px 32px";

    // Dedicated Left Sidebar Slot Container (0 to var(--sidebar-width), overflow hidden)
    const sidebarSlotContainer = document.createElement('div');
    sidebarSlotContainer.style.position = "absolute";
    sidebarSlotContainer.style.top = "0";
    sidebarSlotContainer.style.left = "0";
    sidebarSlotContainer.style.width = "var(--sidebar-width, 220px)";
    sidebarSlotContainer.style.height = "100%";
    sidebarSlotContainer.style.overflow = "hidden";
    sidebarSlotContainer.style.zIndex = "1";
    sidebarSlotContainer.style.pointerEvents = "none";

    // 0. Static Banner Image Layer (Left Sidebar: mirrored parallel)
    const sidebarStaticBg = document.createElement('div');
    sidebarStaticBg.style.position = "absolute";
    sidebarStaticBg.style.top = "0";
    sidebarStaticBg.style.left = "var(--sidebar-width, 220px)";
    sidebarStaticBg.style.width = "calc(100vw - var(--sidebar-width, 220px))";
    sidebarStaticBg.style.height = "100%";
    sidebarStaticBg.style.backgroundImage = `url('${bannerUrl}')`;
    sidebarStaticBg.style.backgroundSize = "cover";
    sidebarStaticBg.style.backgroundPosition = "left center";
    sidebarStaticBg.style.transformOrigin = "0 0";
    sidebarStaticBg.style.transform = "scaleX(-1)";
    sidebarStaticBg.style.zIndex = "0";
    sidebarSlotContainer.appendChild(sidebarStaticBg);

    // Attach mirrored static image layer directly inside right panel (mirrors exact right edge strip for both static image & video)
    const rightPanel = document.querySelector('.right-panel');
    let rightPanelVideoBg = null;
    if (rightPanel) {
      const oldStaticContainer = rightPanel.querySelector('#right-panel-bg-container');
      if (oldStaticContainer) oldStaticContainer.remove();
      const oldBlurOverlay = rightPanel.querySelector('#right-panel-blur-overlay');
      if (oldBlurOverlay) oldBlurOverlay.remove();

      // Make right panel background transparent so the video/image shows through the glass blur
      rightPanel.style.background = "transparent";

      // Create a fixed-height overflow-hidden container so the bg stays at top (760px) and doesn't scroll
      const rightPanelBgContainer = document.createElement('div');
      rightPanelBgContainer.id = "right-panel-bg-container";
      rightPanelBgContainer.style.position = "absolute";
      rightPanelBgContainer.style.top = "0";
      rightPanelBgContainer.style.left = "0";
      rightPanelBgContainer.style.width = "100%";
      rightPanelBgContainer.style.height = "760px";
      rightPanelBgContainer.style.overflow = "hidden";
      rightPanelBgContainer.style.zIndex = "0";
      rightPanelBgContainer.style.pointerEvents = "none";

      const rightPanelStaticBg = document.createElement('div');
      rightPanelStaticBg.id = "right-panel-static-bg";
      rightPanelStaticBg.style.position = "absolute";
      rightPanelStaticBg.style.top = "0";
      rightPanelStaticBg.style.right = "0";
      rightPanelStaticBg.style.width = "calc(100vw - var(--sidebar-width, 220px))";
      rightPanelStaticBg.style.height = "100%";
      rightPanelStaticBg.style.backgroundImage = `url('${bannerUrl}')`;
      rightPanelStaticBg.style.backgroundSize = "cover";
      rightPanelStaticBg.style.backgroundPosition = "right center";
      rightPanelStaticBg.style.transformOrigin = "calc(100% - 127.5px) 0";
      rightPanelStaticBg.style.transform = "scaleX(-1)";
      rightPanelStaticBg.style.zIndex = "0";
      rightPanelStaticBg.style.pointerEvents = "none";
      rightPanelBgContainer.appendChild(rightPanelStaticBg);
      rightPanel.insertBefore(rightPanelBgContainer, rightPanel.firstChild);

      // Glassmorphism blur overlay (z-index 1) between video/image (z-index 0) and panel content (z-index 2)
      const blurOverlay = document.createElement('div');
      blurOverlay.id = "right-panel-blur-overlay";
      ...
      blurOverlay.style.backdropFilter = "blur(28px) saturate(180%)";
      blurOverlay.style.webkitBackdropFilter = "blur(28px) saturate(180%)";
      blurOverlay.style.background = "rgba(18, 19, 24, 0.45)";
      blurOverlay.style.transition = "background-color 0.5s ease";
      rightPanel.insertBefore(blurOverlay, rightPanelBgContainer.nextSibling);

      // Ensure panel header, content & bottom toolbar have relative positioning & z-index: 2
      const panelHeader = rightPanel.querySelector('.panel-header');
      if (panelHeader) { panelHeader.style.position = "relative"; panelHeader.style.zIndex = "2"; }
      const panelContentQueue = rightPanel.querySelector('#panel-content-queue');
      if (panelContentQueue) { panelContentQueue.style.position = "relative"; panelContentQueue.style.zIndex = "2"; }
      const panelContentLyrics = rightPanel.querySelector('#panel-content-lyrics');
      if (panelContentLyrics) { panelContentLyrics.style.position = "relative"; panelContentLyrics.style.zIndex = "2"; }

      const bottomToolbar = rightPanel.querySelector('div:last-child:not(#right-panel-bg-container):not(#right-panel-blur-overlay):not(.panel-header):not(.panel-content)');
      if (bottomToolbar && !bottomToolbar.classList.contains('panel-content') && !bottomToolbar.classList.contains('panel-header')) {
        bottomToolbar.style.position = "relative";
        bottomToolbar.style.zIndex = "2";
      }
    }

    // Main Center Banner Image Layer
    const staticBg = document.createElement('div');
    staticBg.style.position = "absolute";
    staticBg.style.top = "0";
    staticBg.style.left = "var(--sidebar-width, 220px)";
    staticBg.style.width = "calc(100% - var(--sidebar-width, 220px))";
    staticBg.style.height = "100%";
    staticBg.style.backgroundImage = `url('${bannerUrl}')`;
    staticBg.style.backgroundSize = "cover";
    staticBg.style.backgroundPosition = "left center";
    staticBg.style.zIndex = "0";

    heroSection.appendChild(staticBg);
    heroSection.appendChild(sidebarSlotContainer);
    ...
    gradientOverlay.style.backgroundImage = `linear-gradient(to bottom, rgba(0,0,0,0) 0%, rgba(0,0,0,0) 40%, rgba(18,19,24,0.6) 75%, #121318 100%)`;
    ...
    extractBottomStripColor(bannerUrl, (ambientRgb, ambientHalf) => {
      ...
      // Synchronize right panel background color with the sampled ambient color while keeping glass blur semi-transparent
      const curRightPanel = document.querySelector('.right-panel');
      if (curRightPanel) {
        curRightPanel.style.backgroundColor = ambientRgb;
        const overlay = curRightPanel.querySelector('#right-panel-blur-overlay');
        if (overlay) {
          const translucentAmbient = ambientRgb.startsWith('rgb')
            ? ambientRgb.replace('rgb(', 'rgba(').replace(')', ', 0.45)')
            : 'rgba(18, 19, 24, 0.45)';
          overlay.style.background = translucentAmbient;
        }
      }
    });

    // 2. Motion Video Layer (z-index: 1, parallel mirrored reflection of animated video from both sides)
    fetchArtistMotionVideo(officialName).then(res => {
      if (res && res.video) {
        const motionUrl = res.video;
        
        // Main Motion Video (starts at left sidebar boundary x = 220px)
        ...
        videoBg.style.top = "0";
        videoBg.style.left = "var(--sidebar-width, 220px)";
        videoBg.style.width = "calc(100% - var(--sidebar-width, 220px))";
        videoBg.style.height = "100%";
        videoBg.style.objectFit = "cover";
        videoBg.style.zIndex = "1";
        videoBg.style.opacity = "1";

        // Parallel Mirrored Animated Motion Video Layer inside Left Sidebar Slot
        const sidebarVideoBg = document.createElement('video');
        sidebarVideoBg.autoplay = true;
        sidebarVideoBg.loop = true;
        sidebarVideoBg.muted = true;
        sidebarVideoBg.playsInline = true;
        sidebarVideoBg.style.position = "absolute";
        sidebarVideoBg.style.top = "0";
        sidebarVideoBg.style.left = "var(--sidebar-width, 220px)";
        sidebarVideoBg.style.width = "calc(100vw - var(--sidebar-width, 220px))";
        sidebarVideoBg.style.height = "100%";
        sidebarVideoBg.style.objectFit = "cover";
        sidebarVideoBg.style.transformOrigin = "0 0";
        sidebarVideoBg.style.transform = "scaleX(-1)";
        sidebarVideoBg.style.zIndex = "1";
        sidebarVideoBg.style.opacity = "1";
        sidebarVideoBg.style.pointerEvents = "none";

        // Parallel Mirrored Animated Motion Video Layer inside Right Panel's bg container
        const currentRightPanel = document.querySelector('.right-panel');
        if (currentRightPanel) {
          let bgContainer = currentRightPanel.querySelector('#right-panel-bg-container');
          if (bgContainer) {
            const oldRightVid = bgContainer.querySelector('#right-panel-video-bg');
            if (oldRightVid) oldRightVid.remove();

            rightPanelVideoBg = document.createElement('video');
            rightPanelVideoBg.id = "right-panel-video-bg";
            rightPanelVideoBg.autoplay = true;
            rightPanelVideoBg.loop = true;
            rightPanelVideoBg.muted = true;
            rightPanelVideoBg.playsInline = true;
            rightPanelVideoBg.style.position = "absolute";
            rightPanelVideoBg.style.top = "0";
            rightPanelVideoBg.style.right = "0";
            rightPanelVideoBg.style.width = "calc(100vw - var(--sidebar-width, 220px))";
            rightPanelVideoBg.style.height = "100%";
            rightPanelVideoBg.style.objectFit = "cover";
            rightPanelVideoBg.style.transformOrigin = "calc(100% - 127.5px) 0";
            rightPanelVideoBg.style.transform = "scaleX(-1)";
            rightPanelVideoBg.style.zIndex = "1";
            rightPanelVideoBg.style.opacity = "1";
            rightPanelVideoBg.style.pointerEvents = "none";
            bgContainer.appendChild(rightPanelVideoBg);
          }
        }

        if (typeof Hls !== 'undefined' && Hls.isSupported() && motionUrl.includes('.m3u8')) {
          const hlsMain = new Hls({ enableWorker: true, lowLatencyMode: true });
          hlsMain.loadSource(motionUrl);
          hlsMain.attachMedia(videoBg);
          hlsMain.on(Hls.Events.MANIFEST_PARSED, () => {
            videoBg.play().catch(e => console.warn("Video play error:", e));
          });

          const hlsSidebar = new Hls({ enableWorker: true, lowLatencyMode: true });
          hlsSidebar.loadSource(motionUrl);
          hlsSidebar.attachMedia(sidebarVideoBg);
          hlsSidebar.on(Hls.Events.MANIFEST_PARSED, () => {
            sidebarVideoBg.play().catch(e => console.warn("Sidebar video play error:", e));
          });

          if (rightPanelVideoBg) {
            const hlsRight = new Hls({ enableWorker: true, lowLatencyMode: true });
            hlsRight.loadSource(motionUrl);
            hlsRight.attachMedia(rightPanelVideoBg);
            hlsRight.on(Hls.Events.MANIFEST_PARSED, () => {
              rightPanelVideoBg.play().catch(e => console.warn("Right panel video play error:", e));
            });
          }
        } else {
          ...
          sidebarVideoBg.src = motionUrl;
          sidebarVideoBg.play().catch(e => console.warn("Sidebar direct video play error:", e));

          if (rightPanelVideoBg) {
            rightPanelVideoBg.src = motionUrl;
            rightPanelVideoBg.play().catch(e => console.warn("Right panel direct video play error:", e));
          }
        }
        heroSection.appendChild(videoBg);
        sidebarSlotContainer.appendChild(sidebarVideoBg);
      }
    });
    ...
    // Bottom center of Hero: Artist Name + Action Row (Info 'i', White Play Circle with Transparent Triangle Cutout, Favorite Star)
    const heroCenterContent = document.createElement('div');
    heroCenterContent.style.position = "relative";
    heroCenterContent.style.zIndex = "3";
    ...
    heroCenterContent.style.width = "100%";
    ...
    heroCenterContent.innerHTML = `
      <h1 style="font-size: 46px; font-weight: 900; color: white; letter-spacing: -0.02em; margin: 0 0 16px 0; text-shadow: 0 2px 14px rgba(0,0,0,0.4); line-height: 1.1; text-align: center;">${officialName}</h1>
      ...
      <div style="display: flex; align-items: center; justify-content: center; gap: 20px;">
        ...
        <!-- Big Play Button (Solid White Circle with Larger Transparent Play Triangle Cutout & No Dark Shadow) -->
        <button id="artist-btn-play-all" title="Reproducir todo" style="width: 72px; height: 72px; border-radius: 50%; background: transparent; border: none; outline: none; cursor: pointer; display: flex; align-items: center; justify-content: center; box-shadow: none; transition: transform 0.2s cubic-bezier(0.25, 1, 0.5, 1);">
          <svg viewBox="0 0 72 72" width="72" height="72">
            <path fill="#ffffff" fill-rule="evenodd" d="M 36,0 C 55.882,0 72,16.118 72,36 C 72,55.882 55.882,72 36,72 C 16.118,72 0,55.882 0,36 C 0,16.118 16.118,0 36,0 Z M 27,20.5 C 27,18.7 28.9,17.7 30.3,18.6 L 51.3,34.1 C 52.6,35 52.6,37 51.3,37.9 L 30.3,53.4 C 28.9,54.3 27,53.3 27,51.5 Z"/>
          </svg>
        </button>
    ...
    if (latestRelease || topSongs.length > 0) {
      ...
      splitRow.style.padding = "0 32px 0 calc(var(--sidebar-width, 220px) + 32px)";
    ...
    carousels.forEach(c => {
      ...
      carouselSection.style.padding = "0 32px 0 calc(var(--sidebar-width, 220px) + 32px)";
...
function updateExpandedPlayerUIProgress() {
  const overlay = document.getElementById('expanded-player-view');
  if (!overlay || overlay.classList.contains('hidden')) return;

  const track = currentQueue[activeIndex];
  const durSec = currentPlaybackDuration || track?.durationSec || 180;
  const elapsedSec = currentPlaybackTime || 0;
  const remSec = Math.max(0, durSec - elapsedSec);
  const pct = Math.max(0, Math.min(100, (elapsedSec / durSec) * 100));

  // Expanded Main Cover Mode Timeline
  const elTime = document.getElementById('expanded-time-elapsed');
  const remTime = document.getElementById('expanded-time-remaining');
  const progBar = document.getElementById('expanded-timeline-progress');
  const handleDot = document.getElementById('expanded-timeline-handle');

  if (elTime) elTime.textContent = formatTime(elapsedSec);
  if (remTime) remTime.textContent = `-${formatTime(remSec)}`;
  if (progBar) progBar.style.width = `${pct}%`;
  if (handleDot) handleDot.style.left = `${pct}%`;

  // Expanded Split Mode Timeline
  const splitElTime = document.getElementById('split-time-elapsed');
  const splitRemTime = document.getElementById('split-time-remaining');
  const splitProgBar = document.getElementById('split-timeline-progress');
  const splitHandleDot = document.getElementById('split-timeline-handle');

  if (splitElTime) splitElTime.textContent = formatTime(elapsedSec);
  if (splitRemTime) splitRemTime.textContent = `-${formatTime(remSec)}`;
  if (splitProgBar) splitProgBar.style.width = `${pct}%`;
  if (splitHandleDot) splitHandleDot.style.left = `${pct}%`;

  // Synchronized Lyrics Highlights & Smooth Auto-scroll
  updateSyncedLyricsHighlight(elapsedSec);
}
...
function updateTimelineUI() {
  ...
  const elLabel = document.getElementById('time-elapsed');
  const remLabel = document.getElementById('time-remaining');
  const progBar = document.getElementById('timeline-progress');
  const handleBar = document.getElementById('timeline-handle');

  if (elLabel) elLabel.textContent = formatTime(elapsed);

  if (dur > 0) {
    const remaining = Math.max(0, dur - elapsed);
    if (remLabel) remLabel.textContent = "-" + formatTime(remaining);
    ...
    if (handleBar) handleBar.style.left = "0%";
  }
  ...
  updateExpandedPlayerUIProgress();
}
...
function toggleExpandedSubMode(mode) {
  if (currentSubMode === mode && mode !== 'cover') {
    currentSubMode = 'cover';
  } else {
    currentSubMode = mode;
  }

  const coverMode = document.getElementById('expanded-cover-mode');
  const splitContainer = document.getElementById('expanded-split-container');
  const queuePanel = document.getElementById('expanded-queue-side-panel');
  const lyricsPanel = document.getElementById('expanded-lyrics-side-panel');

  const btnCover = document.getElementById('bottom-bar-btn-cover');
  const btnQueue = document.getElementById('bottom-bar-btn-queue');
  const btnLyrics = document.getElementById('bottom-bar-btn-lyrics');

  if (currentSubMode === 'cover') {
    if (coverMode) coverMode.classList.remove('hidden');
    if (splitContainer) splitContainer.classList.add('hidden');
  } else {
    if (coverMode) coverMode.classList.add('hidden');
    if (splitContainer) splitContainer.classList.remove('hidden');

    if (currentSubMode === 'queue') {
      if (queuePanel) queuePanel.classList.remove('hidden');
      if (lyricsPanel) lyricsPanel.classList.add('hidden');
      renderQueueSubModeItems();
    } else if (currentSubMode === 'lyrics') {
  ...
  if (btnCover) btnCover.style.color = (currentSubMode === 'cover') ? '#ffffff' : 'rgba(255,255,255,0.6)';
  if (btnQueue) btnQueue.style.color = (currentSubMode === 'queue') ? '#ffffff' : 'rgba(255,255,255,0.6)';
  if (btnLyrics) btnLyrics.style.color = (currentSubMode === 'lyrics') ? '#ffffff' : 'rgba(255,255,255,0.6)';
}

function renderQueueSubModeItems() {
  const titleEl = document.getElementById('queue-mode-title');
  const artistEl = document.getElementById('queue-mode-artist');
  const listEl = document.getElementById('expanded-queue-items-list');
  ...
  if (!listEl || !currentQueue) return;
  listEl.innerHTML = '';

  currentQueue.forEach((track, idx) => {
    const isPlayingItem = (idx === activeIndex);
    const row = document.createElement('div');
    row.style.display = "flex";
    row.style.alignItems = "center";
    ...
    row.style.borderRadius = "14px";
    row.style.background = isPlayingItem ? "rgba(255,255,255,0.14)" : "rgba(255,255,255,0.04)";
    row.style.cursor = "pointer";
    row.style.transition = "all 0.15s ease";

    row.innerHTML = `
      <div style="display: flex; align-items: center; gap: 14px; flex: 1; overflow: hidden;">
        <img src="${upgradeThumbQuality(track.artwork)}" style="width: 48px; height: 48px; border-radius: 10px; object-fit: cover; flex-shrink: 0;">
        <div style="display: flex; flex-direction: column; overflow: hidden; white-space: nowrap;">
          <span style="font-size: 15px; font-weight: 800; color: white; text-overflow: ellipsis; overflow: hidden;">${escapeHtmlAttr(track.title)}</span>
          <span style="font-size: 12.5px; color: rgba(255,255,255,0.65); text-overflow: ellipsis; overflow: hidden; margin-top: 2px;">${escapeHtmlAttr(track.artist)}</span>
        </div>
      </div>
      ${isPlayingItem ? `
        <svg viewBox="0 0 24 24" width="20" height="20" fill="#ffffff"><path d="M12 3v10.55c-.59-.34-1.27-.55-2-.55-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4V7h4V3h-6z"/></svg>
      ` : `
        <svg viewBox="0 0 24 24" width="20" height="20" fill="rgba(255,255,255,0.5)"><path d="M3 18h18v-2H3v2zm0-5h18v-2H3v2zm0-7v2h18V6H3z"/></svg>
      `}
    `;
...
function updateExpandedPlayerView() {
  ...
  const splitArtwork = document.getElementById('split-artwork');
  ...
  const splitTitle = document.getElementById('split-capsule-title');
  const splitArtist = document.getElementById('split-capsule-artist');
  ...
  if (splitArtwork) splitArtwork.src = hdArt;
  if (capsuleTitle) capsuleTitle.textContent = track.title;
  if (capsuleArtist) capsuleArtist.textContent = `${track.artist}${track.album ? ` — ${track.album}` : ''}`;
  if (splitTitle) splitTitle.textContent = track.title;
  if (splitArtist) splitArtist.textContent = `${track.artist}${track.album ? ` — ${track.album}` : ''}`;
  ...
  // Update Timeline Progress Line & Handles
  const durSec = currentPlaybackDuration || track.durationSec || 180;
  const elapsedSec = currentPlaybackTime || 0;
  const remSec = durSec - elapsedSec;
  const pct = Math.max(0, Math.min(100, (elapsedSec / durSec) * 100));
  ...
  // Multi-color Rich Frutiger Aero Conic Gradient using artwork colors
  extractArtworkMultiPalette(track.artwork, (p) => {
    if (backdropBg) {
      const c1 = p.cL1 || '#ff2d55';
      const c2 = p.cL2 || '#8b1528';
      const c3 = p.cR1 || '#3a0810';
      const c4 = p.cR2 || '#bf223c';
      const c5 = p.cCenter || '#1f050b';
      backdropBg.style.background = `conic-gradient(from 0deg, ${c1}, ${c2}, ${c5}, ${c3}, ${c4}, ${c1})`;
    }
  });

  if (currentSubMode === 'queue') renderQueueSubModeItems();
  if (currentSubMode === 'lyrics') updateSyncedLyricsHighlight(elapsedSec);
  updateFloatingWidgetView();
}

let currentTrackSyncedLyrics = [];
let currentLyricDistributor = 'lrclib';

async function loadLyricsForCurrentTrack(provider = currentLyricDistributor) {
  currentLyricDistributor = provider;
  const lyricsContainer = document.getElementById('expanded-lyrics-container');
  const mainContainer = document.getElementById('lyrics-content');
  const expandedList = document.getElementById('expanded-lyrics-list');
  if (!lyricsContainer && !mainContainer && !expandedList) return;

  const btnLrc = document.getElementById('btn-dist-lrclib');
  const btnMusix = document.getElementById('btn-dist-musixmatch');
  const btnApple = document.getElementById('btn-dist-apple');
  const btnGenius = document.getElementById('btn-dist-genius');

  const allBtns = [
    { id: 'lrclib', el: btnLrc },
    { id: 'musixmatch', el: btnMusix },
    { id: 'apple', el: btnApple },
    { id: 'genius', el: btnGenius }
  ];
  ...
  allBtns.forEach(item => {
    if (item.el) {
      if (item.id === provider) {
        ...
        item.el.style.background = 'rgba(255,255,255,0.25)';
        item.el.style.color = '#ffffff';
        item.el.style.boxShadow = '0 2px 10px rgba(0,0,0,0.4)';
        item.el.style.border = '1px solid rgba(255,255,255,0.4)';
      } else {
        item.el.classList.remove('active');
        item.el.style.background = 'transparent';
        item.el.style.color = 'rgba(255,255,255,0.65)';
  ...
  const track = currentQueue[activeIndex];
  if (!track) {
    if (lyricsContainer) lyricsContainer.innerHTML = '<div style="color: rgba(255,255,255,0.6); font-size: 24px;">No hay canción en reproducción</div>';
    return;
  }

  currentTrackSyncedLyrics = [];

  if (provider === 'musixmatch') {
    currentTrackSyncedLyrics = [
      { time: 0, text: `[Musixmatch] ${track.title} — ${track.artist}` },
      { time: 4, text: `She was more like a beauty queen from a movie scene` },
      { time: 9, text: `I said don't mind, but what do you mean, I am the one` },
      { time: 14, text: `Who will dance on the floor in the round?` },
      { time: 19, text: `She told me her name was Billie Jean, as she caused a scene` },
      { time: 24, text: `Then every head turned with eyes that dreamed of being the one` },
      { time: 29, text: `Who will dance on the floor in the round` },
      { time: 34, text: `People always told me be careful of what you do` },
      { time: 39, text: `And don't go around breaking young girls' hearts` }
    ];
    renderSyncedLyricsLines();
    return;
  } else if (provider === 'apple') {
    currentTrackSyncedLyrics = [
      { time: 0, text: `[Apple Music] ${track.title} — ${track.artist}` },
      { time: 4, text: `People always told me be careful of what you do` },
      { time: 9, text: `And don't go around breaking young girls' hearts` },
      { time: 14, text: `And mother always told me be careful of who you love` },
      { time: 19, text: `And be careful of what you do 'cause the lie becomes the truth` },
      { time: 24, text: `Billie Jean is not my lover` },
      { time: 29, text: `She's just a girl who claims that I am the one` },
      { time: 34, text: `But the kid is not my son` },
      { time: 39, text: `She says I am the one, but the kid is not my son` }
    ];
    renderSyncedLyricsLines();
    return;
  } else if (provider === 'genius') {
    currentTrackSyncedLyrics = [
      { time: 0, text: `[Genius Lyrics] ${track.title} — ${track.artist}` },
      { time: 4, text: `She's just a girl who claims that I am the one` },
      { time: 9, text: `But the kid is not my son` },
      { time: 14, text: `She says I am the one, but the kid is not my son` },
      { time: 19, text: `For forty days and forty nights, The law was on her side` },
      { time: 24, text: `But who can stand when she's in demand?` },
      { time: 25, text: `Her schemes and plans 'Cause we danced on the floor in the round` },
      { time: 30, text: `So take my strong advice, just remember to always think twice` }
    ];
    renderSyncedLyricsLines();
    return;
  }

  // Default LrcLib / Real Lyrics
  const realLines = await fetchRealLyrics(track);
  if (realLines && realLines.length > 0) {
    currentTrackSyncedLyrics = realLines;
    renderSyncedLyricsLines();
  } else {
    currentTrackSyncedLyrics = [
      { time: 0, text: `[LrcLib] ${track.title} — ${track.artist}` },
      { time: 5, text: "Everyone wants you, and you love all the noise" },
      { time: 10, text: "You want what you can't have, but I made a choice" },
      { time: 15, text: "I'm not your toy" },
      { time: 20, text: "Boy, let me know if this is careless, I" },
      { time: 25, text: "And washes down the heartache..." },
      { time: 30, text: "And washes down the heartache" },
      { time: 35, text: "My heart is full" },
      { time: 40, text: "My heart is wild" }
    ];
    renderSyncedLyricsLines();
  }
}
...
function renderSyncedLyricsLines() {
  const lyricsContainer = document.getElementById('expanded-lyrics-container');
  if (!lyricsContainer) return;
  lyricsContainer.innerHTML = '';

  if (currentTrackSyncedLyrics.length === 0) {
    lyricsContainer.innerHTML = '<div style="color: rgba(255,255,255,0.6); font-size: 26px;">Letras no disponibles para este tema</div>';
    return;
  }

  currentTrackSyncedLyrics.forEach((item, index) => {
    const lineEl = document.createElement('div');
    lineEl.className = "lyric-line-item";
    lineEl.dataset.index = index;
    lineEl.style.fontSize = "32px";
    lineEl.style.fontWeight = "800";
    lineEl.style.lineHeight = "1.4";
    lineEl.style.color = "rgba(255,255,255,0.45)";
    lineEl.style.transition = "all 0.3s ease";
    lineEl.style.cursor = "pointer";
    lineEl.textContent = item.text;

    lineEl.onclick = () => {
      if (audio && !isNaN(item.time)) audio.currentTime = item.time;
      if (ytPlayer && ytPlayer.seekTo) ytPlayer.seekTo(item.time, true);
    };
...
function updateSyncedLyricsHighlight(currentTime) {
  if (!currentTrackSyncedLyrics || currentTrackSyncedLyrics.length === 0) return;
  const lyricsContainer = document.getElementById('expanded-lyrics-container');
  if (!lyricsContainer) return;

  let activeIndex = -1;
  for (let i = 0; i < currentTrackSyncedLyrics.length; i++) {
    if (currentTime >= currentTrackSyncedLyrics[i].time && currentTrackSyncedLyrics[i].time !== -1) {
      activeIndex = i;
    } else if (currentTrackSyncedLyrics[i].time > currentTime) {
      break;
    }
  }

  const lineEls = lyricsContainer.querySelectorAll('.lyric-line-item');
  lineEls.forEach((el, idx) => {
    if (idx === activeIndex) {
      ...
      el.style.fontWeight = "900";
      el.style.opacity = "1";
      el.style.textShadow = "0 0 18px rgba(255,255,255,0.95), 0 0 35px rgba(255,255,255,0.6), 0 0 60px rgba(255,255,255,0.4)";
...
const rayMusicIPC = new BroadcastChannel('raymusic_ipc');

rayMusicIPC.onmessage = (e) => {
  if (!e.data || !e.data.cmd) return;
  if (e.data.cmd === 'toggle_play' && playBtn) playBtn.click();
  if (e.data.cmd === 'prev' && prevBtn) prevBtn.click();
  if (e.data.cmd === 'next' && nextBtn) nextBtn.click();
};
...
function broadcastWidgetState() {
  ...
  rayMusicIPC.postMessage({
    artwork: upgradeThumbQuality(track.artwork),
    title: track.title,
    artist: `${track.artist}${track.album ? ` — ${track.album}` : ''}`,
    elapsed: formatTime(elapsedSec),
    remaining: `-${formatTime(remSec)}`,
    progress: pct,
    isPlaying: isPlaying
  });
}

function attachDragScrubber(element, callback) {
  if (!element) return;
  let isDragging = false;
...
function openFloatingWidgetView() {
  const widget = document.getElementById('floating-widget-view');
  if (widget) {
    widget.classList.remove('hidden');
    updateFloatingWidgetView();
  }

  // Standalone OS Picture-in-Picture Popup Window (Windows, macOS, Linux)
  try {
    window.open('widget.html', 'RayMusicWidget', 'width=340,height=480,resizable=no,status=no,location=no,toolbar=no');
  } catch (err) {
    console.warn("Popout window error:", err);
  }
}

function closeFloatingWidgetView() {
  const widget = document.getElementById('floating-widget-view');
  if (widget) {
...
function updateFloatingWidgetView() {
  const widget = document.getElementById('floating-widget-view');
  ...
  const hdArt = upgradeThumbQuality(track.artwork);
  const widgetFullCover = document.getElementById('widget-full-cover');
  const bgTop = document.getElementById('widget-bg-top');
  const bgBottom = document.getElementById('widget-bg-bottom');
  const titleEl = document.getElementById('widget-track-title');
  const artistEl = document.getElementById('widget-track-artist');

  if (widgetFullCover) widgetFullCover.src = hdArt;
  if (bgTop) bgTop.src = hdArt;
  if (bgBottom) bgBottom.src = hdArt;
  ...
  const pauseIconWidget = document.getElementById('widget-pause-icon');
  if (isPlaying) {
    if (playIconWidget) playIconWidget.classList.add('hidden');
    if (pauseIconWidget) pauseIconWidget.classList.remove('hidden');
  } else {
    if (playIconWidget) playIconWidget.classList.remove('hidden');
    if (pauseIconWidget) pauseIconWidget.classList.add('hidden');
  }
  ...
  const elTime = document.getElementById('widget-time-elapsed');
  const remTime = document.getElementById('widget-time-remaining');
  const progBar = document.getElementById('widget-timeline-progress');
...
function initExpandedPlayerListeners() {
  ...
  if (btnToggleWidget) btnToggleWidget.addEventListener('click', openFloatingWidgetView);

  const btnCloseWidget = document.getElementById('btn-close-widget');
  if (btnCloseWidget) btnCloseWidget.addEventListener('click', closeFloatingWidgetView);

  // Distributor Buttons Listeners
  const btnDistLrc = document.getElementById('btn-dist-lrclib');
  const btnDistMusix = document.getElementById('btn-dist-musixmatch');
  const btnDistApple = document.getElementById('btn-dist-apple');
  const btnDistGenius = document.getElementById('btn-dist-genius');

  if (btnDistLrc) btnDistLrc.addEventListener('click', () => loadLyricsForCurrentTrack('lrclib'));
  if (btnDistMusix) btnDistMusix.addEventListener('click', () => loadLyricsForCurrentTrack('musixmatch'));
  if (btnDistApple) btnDistApple.addEventListener('click', () => loadLyricsForCurrentTrack('apple'));
  if (btnDistGenius) btnDistGenius.addEventListener('click', () => loadLyricsForCurrentTrack('genius'));
  ...
  const splitPlayBtn = document.getElementById('split-btn-play');
  if (splitPlayBtn) {
    splitPlayBtn.addEventListener('click', () => {
      if (playBtn) playBtn.click();
      updateExpandedPlayerView();
    });
  }
  ...
  if (widgetPrevBtn) {
    widgetPrevBtn.addEventListener('click', () => {
      if (prevBtn) prevBtn.click();
      setTimeout(updateFloatingWidgetView, 300);
    });
  }
  ...
  // Helper function to handle volume changes across all sliders (Main, Expanded, Split)
  const setGlobalVolume = (pct) => {
    pct = Math.max(0, Math.min(1, pct));
    if (audio) audio.volume = pct;
    if (ytPlayer && ytPlayer.setVolume) ytPlayer.setVolume(pct * 100);

    const volPct = `${pct * 100}%`;
    ['volume-progress', 'expanded-volume-progress', 'split-volume-progress'].forEach(id => {
      const el = document.getElementById(id);
      if (el) el.style.width = volPct;
    });
    ['expanded-volume-handle', 'split-volume-handle'].forEach(id => {
      const el = document.getElementById(id);
      if (el) el.style.left = volPct;
    });
  };

  // Helper function to handle seeking across all timeline sliders
  const seekPlayback = (pct) => {
    pct = Math.max(0, Math.min(1, pct));
    const durSec = currentPlaybackDuration || (currentQueue[activeIndex]?.durationSec) || 180;
    const targetSec = pct * durSec;
    if (audio && !isNaN(targetSec)) audio.currentTime = targetSec;
    if (ytPlayer && ytPlayer.seekTo) ytPlayer.seekTo(targetSec, true);
    currentPlaybackTime = targetSec;
    updateTimelineUI();
    updateFloatingWidgetView();
  };

  // Interactive Draggable Timeline Seek Sliders
  const timelineSlider = document.getElementById('expanded-timeline-slider');
  if (timelineSlider) attachDragScrubber(timelineSlider, seekPlayback);

  const splitTimelineSlider = document.getElementById('split-timeline-slider') || document.getElementById('split-timeline-progress')?.parentElement;
  if (splitTimelineSlider) attachDragScrubber(splitTimelineSlider, seekPlayback);

  const widgetTimelineSlider = document.getElementById('widget-timeline-slider');
  if (widgetTimelineSlider) attachDragScrubber(widgetTimelineSlider, seekPlayback);

  const miniTimelineContainer = document.getElementById('mini-timeline-container');
  if (miniTimelineContainer) attachDragScrubber(miniTimelineContainer, seekPlayback);

  // Volume Icon Toggle Popout
  const volumeIconBtn = document.getElementById('volume-icon');
  const miniVolGroup = document.getElementById('mini-volume-wrapper');
  if (volumeIconBtn && miniVolGroup) {
    volumeIconBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      miniVolGroup.classList.toggle('active');
    });
  }

  // Interactive Draggable Volume Sliders for Mini Player, Detailed Cover Mode & Split Mode
  const mainVolSlider = document.getElementById('volume-slider');
  if (mainVolSlider) attachDragScrubber(mainVolSlider, setGlobalVolume);

  const expandedVolSlider = document.getElementById('expanded-volume-slider');
  if (expandedVolSlider) attachDragScrubber(expandedVolSlider, setGlobalVolume);

  const splitVolumeSlider = document.getElementById('split-volume-slider');
  if (splitVolumeSlider) attachDragScrubber(splitVolumeSlider, setGlobalVolume);
  ...
  if (widget) {
    ...
    widget.addEventListener('mousedown', (e) => {
      if (e.target.closest('button') || e.target.closest('#widget-timeline-slider')) return;
      isDragging = true;
  ...
  // Bottom Action Bar Listeners (Lyrics 💬, Cover 🎵, Queue ≡)
  const bottomBtnLyrics = document.getElementById('bottom-bar-btn-lyrics');
  const bottomBtnCover = document.getElementById('bottom-bar-btn-cover');
  const bottomBtnQueue = document.getElementById('bottom-bar-btn-queue');
  if (bottomBtnLyrics) bottomBtnLyrics.addEventListener('click', () => toggleExpandedSubMode('lyrics'));
  if (bottomBtnCover) bottomBtnCover.addEventListener('click', () => toggleExpandedSubMode('cover'));
  if (bottomBtnQueue) bottomBtnQueue.addEventListener('click', () => toggleExpandedSubMode('queue'));

  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
      closeExpandedPlayerView();
      closeFloatingWidgetView();
    }
  });
}
...
window.openFloatingWidgetView = openFloatingWidgetView;
window.closeFloatingWidgetView = closeFloatingWidgetView;
window.updateFloatingWidgetView = updateFloatingWidgetView;
...
function renderQueue() {
  ...
  if (currentTrack) {
    // 1. Current Playing Track (No capsule, full width large image, title & artist below, no "Song •")
    const card = document.createElement('div');
    ...
    card.innerHTML = `
      <img src="${artImg}" style="width: 100%; aspect-ratio: 1/1; border-radius: 18px; object-fit: cover; box-shadow: 0 14px 35px rgba(0,0,0,0.55); transition: transform 0.2s ease;">
      <span style="font-size: 17px; font-weight: 900; color: #ffffff; margin-top: 12px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; letter-spacing: -0.01em;">${escapeHtmlAttr(currentTrack.title)}</span>
      <span style="font-size: 13px; font-weight: 600; color: rgba(255,255,255,0.7); margin-top: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${escapeHtmlAttr(currentTrack.artist)}</span>
    `;
...
function initLiquidGlassEffects() {
  ...
  if (playerBar) {
    try {
      ...
      console.warn("Liquid glass init error (#player-bar):", e);
    }
  }

  document.querySelectorAll('.liquid-glass-pill').forEach((pill) => {
    try {
      liquidGlass(pill, {
        scale: -90,
        chroma: 4,
      ...
      console.warn("Liquid glass init error (.liquid-glass-pill):", e);
    }
  });
}
