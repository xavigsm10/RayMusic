...
async function loadArtistPage(artistId, artistName, shouldPushHistory = true) {
  ...
  try {
    ...
    // 0. Static Banner Image Layer (Main: middle area; Left Sidebar: mirrored parallel)
    ...
    // Attach mirrored static image layer directly inside right panel (mirrors exact red box strip from seam line)
    const rightPanel = document.querySelector('.right-panel');
    if (rightPanel) {
      const oldStatic = rightPanel.querySelector('#right-panel-static-bg');
      ...
      rightPanelStaticBg.style.filter = "blur(18px) brightness(0.7)";
      rightPanelStaticBg.style.pointerEvents = "none";
      rightPanel.insertBefore(rightPanelStaticBg, rightPanel.firstChild);
    }
    ...
    fetchArtistMotionVideo(officialName).then(res => {
      if (res && res.video) {
        ...
        // Attach mirrored motion video layer directly inside right panel element so main-content carousels remain 100% unobstructed
        const currentRightPanel = document.querySelector('.right-panel');
        let rightPanelVideoBg = null;
        if (currentRightPanel) {
          const oldRightVid = currentRightPanel.querySelector('#right-panel-video-bg');
          ...
          rightPanelVideoBg.style.filter = "blur(18px) brightness(0.7)";
          rightPanelVideoBg.style.pointerEvents = "none";
          currentRightPanel.insertBefore(rightPanelVideoBg, currentRightPanel.firstChild);
        }
